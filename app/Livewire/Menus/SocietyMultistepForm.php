...
class SocietyMultistepForm extends Component
{
    ...
    protected $rules = [
        1 => [
            ...
            'signed_member_csv_file' => 'nullable|file|mimes:csv,txt',
        ],
        2 => [],
        3 => []
    ];
    ...
    public function save()
    {
        if ($this->currentStep == 1) {
            ...
            if ($existingSociety && $existingSociety->id !== $this->societyId) {
                // If it already has CSV flats uploaded, it's a completed society
                if (SocietyDetail::where('society_id', $existingSociety->id)->exists()) {
                    ...
                    // It was abandoned before CSV upload. Auto-resume to avoid creating duplicates.
