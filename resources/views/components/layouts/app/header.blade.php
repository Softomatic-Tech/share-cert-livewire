...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark">
    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <a href="{{ route(auth()->user()->role->role === 'Super Admin' ? 'superadmin.dashboard' : (auth()->user()->role->role === 'Admin' ? 'admin.dashboard' : 'user.dashboard')) }}" class="ml-2 mr-5 flex items-center space-x-2 lg:ml-0" wire:navigate>
                <x-app-logo />
            </a>
        ...
        <flux:sidebar stashable sticky class="lg:hidden border-r border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <a href="{{ route(auth()->user()->role->role === 'Super Admin' ? 'superadmin.dashboard' : (auth()->user()->role->role === 'Admin' ? 'admin.dashboard' : 'user.dashboard')) }}" class="ml-1 flex items-center space-x-2" wire:navigate>
                <x-app-logo />
            </a>

            <flux:navlist variant="outline">
                {{-- <flux:navlist.group> --}}
                    <flux:navlist.item icon="layout-grid" :href="route(auth()->user()->role->role === 'Super Admin' ? 'superadmin.dashboard' : (auth()->user()->role->role === 'Admin' ? 'admin.dashboard' : 'user.dashboard'))" 
                    :current="request()->routeIs('superadmin.dashboard') || request()->routeIs('admin.dashboard') || request()->routeIs('user.dashboard')" wire:navigate>
                    {{ __('Home') }}
                    </flux:navlist.item>
