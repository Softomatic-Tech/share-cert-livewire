    ...
    @else
        @foreach ($societyDetails as $details)
            @php
                ...
                $apartmentPassword = data_get($statusData, 'password');
            ...
            <div class="rounded-lg border shadow-sm hover:shadow-lg transition-shadow my-4">
                <div class="grid grid-cols-1 md:grid-cols-2 border-b relative">
                    <div class="p-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <div
                                    class="flex gap-2 overflow-x-auto xl:justify-end pb-1 w-full mt-2 xl:mt-0 items-center">
                                    <div class="flex items-center gap-2 flex-wrap">
                                        ...
                                        @if ($apartmentPassword)
                                            <span
                                                class="inline-flex items-center gap-1 rounded-sm px-2 py-0.5 text-xs font-bold bg-green-100 text-green-700">
                                                Password: {{ $apartmentPassword }}
                                            </span>
                                        @endif
                                    </div>

                                    @if ($details->byeLawCase && $details->byeLawCase->membership_case)
                                        <flux:badge color="red" size="sm">
                                            Case:
                                            {{ match ($details->byeLawCase->membership_case) {
                                                'case_a' => 'Original Membership',
                                                'case_b' => 'Transfer',
                                                'case_c' => 'Nominee Succession',
                                                'case_d' => 'Heir Succession',
                                                default => '',
                                            } }}
                                        </flux:badge>
                                    @endif
                                ...
                                <div class="flex flex-wrap gap-2 mt-1">
                                    @if ($details->no_of_shares)
                                        <span
                                            class="bg-gray-50 text-gray-600 text-[10px] font-bold px-2 py-0.5 rounded-full dark:bg-gray-700 dark:text-gray-300 border border-gray-100 dark:border-gray-600">
                        ...
                        {{-- Owners Info --}}
                        <div
                            class="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-2 border-t border-gray-50 dark:border-gray-700">
                            @for ($i = 1; $i <= 3; $i++)
                                @php
                                    $name = "owner{$i}_name";
                                    $phone = "owner{$i}_mobile";
                                @endphp
                                @if ($details->$name)
                                    <div class="p-2 rounded-lg bg-gray-50/50 dark:bg-gray-700/30">
                                        <p class="text-[9px] uppercase font-bold text-gray-400 mb-0.5 tracking-wider">
                                            Owner {{ $i }}</p>
                            ...
                            <div class="w-full">
                                <div data-dui-stepper-container data-dui-initial-step="1" class="w-full mb-4">
                                    <div class="flex items-center justify-between">
                                        @foreach ($steps as $i => $task)
                                            <div aria-disabled="false" data-dui-step
                                                class="group w-full flex items-center">
                                                <div class="relative">
                                                    <span
                                                        class="relative grid h-8 w-8 place-items-center rounded-full {{ $colors[$i] }}">
                                                    ...
                                                    @endphp
                                                    <span
                                                        class="absolute -bottom-8 start-0 whitespace-wrap text-[10px] {{ in_array($task['Status'], ['Pending', 'Rejected']) ? 'text-stone-500 font-normal' : 'text-stone-800 font-extrabold' }} dark:text-white">{{ $label }}
                                                    </span>
                                                </div>
                                                @if (!$loop->last)
                    ...
                    <div class="absolute top-0 right-0 flex space-x-2">
                        @if ($showCertificateDownloadButton)
                            <flux:tooltip content="View Certificate">
                                <button class="font-bold px-2 py-1 border rounded-md"
                                    onclick="window.open('{{ route('admin.certificate.view', ['id' => $details->id]) }}', '_blank')">
                        ...
                        <flux:tooltip content="Edit Apartment">
                            <button class="font-bold px-2 py-1 border rounded-md"
                                wire:click="fetchOwnersDetail('{{ $details->id }}')">
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-2 p-2">
                    ...
                    @if ($details->agreementCopy)
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->agreementCopy);
                            $isApproved = $this->getFileStatus($statusData, $details->agreementCopy);
                        @endphp
                        <div class="flex items-center text-xs cursor-pointer"
                            wire:click="viewDocument('{{ $details->id }}','{{ $fileUrl }}','{{ $isApproved }}')">
                            @if ($isApproved == 'Approved')
                                <span class="mr-2 flex items-center justify-center"><i
                                        class="fa-regular fa-circle-check text-green-600"></i></span>
                            @else
                                <span class="mr-2 flex items-center justify-center"><i
                                        class="fa-regular fa-circle-xmark text-red-600"></i></span>
                            @endif
                            Copy of Agreement
                        </div>
                    @else
                    ...
                    @if ($details->memberShipForm)
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->memberShipForm);
                    ...
                    @if ($details->allotmentLetter)
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->allotmentLetter);
                    ...
                    @if ($details->possessionLetter)
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->possessionLetter);
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-2 p-3">
                    @if ($details->byeLawCase && $details->byeLawCase->membership_case == 'case_a')
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->byeLawCase->allotmentMembershipLetter);
                            $isApproved = $this->getFileStatus(
                                $statusData,
                                $details->byeLawCase->allotmentMembershipLetter,
                            );
                        @endphp
                        <div class="flex items-center text-xs cursor-pointer"
                            wire:click="viewDocument('{{ $details->id }}','{{ $fileUrl }}','{{ $isApproved }}')">
                    ...
                    @if ($details->byeLawCase && $details->byeLawCase->membership_case == 'case_b')
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->byeLawCase->stampDutyProof);
                        ...
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->byeLawCase->transferorSignature);
                    ...
                    @if ($details->byeLawCase && $details->byeLawCase->membership_case == 'case_c')
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->byeLawCase->deathCertificate);
                        ...
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->byeLawCase->nominationRecord);
                    ...
                    @if ($details->byeLawCase && $details->byeLawCase->membership_case == 'case_d')
                        @php
                            $fileUrl = asset('storage/society_docs/' . $details->byeLawCase->successionCertificate);
                            $isApproved = $this->getFileStatus(
                                $statusData,
                                $details->byeLawCase->successionCertificate,
                            );
                        @endphp
                        <div class="flex items-center text-xs cursor-pointer"
                            wire:click="viewDocument('{{ $details->id }}','{{ $fileUrl }}','{{ $isApproved }}')">
                ...
                <div class="grid grid-cols-1">
                    @php
                        if ($details->byeLawCase && $details->byeLawCase->membership_case == 'case_a') {
                            $allApproved = $this->areAllFilesApproved($statusData, [
                                $details->agreementCopy,
                                $details->memberShipForm,
                                $details->allotmentLetter,
                                $details->possessionLetter,
                                $details->byeLawCase->allotmentMembershipLetter,
                                'Appendix 2',
                                'Appendix 3',
                            ]);
                        } elseif ($details->byeLawCase && $details->byeLawCase->membership_case == 'case_b') {
                            $allApproved = $this->areAllFilesApproved($statusData, [
                                $details->agreementCopy,
                                $details->memberShipForm,
                                $details->allotmentLetter,
                                $details->possessionLetter,
                                $details->byeLawCase->stampDutyProof,
                                $details->byeLawCase->transferorSignature,
                                'Appendix 20(1)',
                                'Appendix 20(2)',
                                'Appendix 21',
                            ]);
                        } elseif ($details->byeLawCase && $details->byeLawCase->membership_case == 'case_c') {
                            $allApproved = $this->areAllFilesApproved($statusData, [
                                $details->agreementCopy,
                                $details->memberShipForm,
                                $details->allotmentLetter,
                                $details->possessionLetter,
                                $details->byeLawCase->deathCertificate,
                                $details->byeLawCase->nominationRecord,
                                'Appendix 15',
                            ]);
                        } elseif ($details->byeLawCase && $details->byeLawCase->membership_case == 'case_d') {
                            $allApproved = $this->areAllFilesApproved($statusData, [
                                $details->agreementCopy,
                                $details->memberShipForm,
                                $details->allotmentLetter,
                    ...
                    @endphp
                    @if ($showVerificationButton)
                        <button class="rounded-md p-2 text-md font-medium cursor-pointer bg-blue-500"
                            wire:click="setDocument('{{ $details->id }}')">Verify Documents</button>
                    @endif
                    @php
                        $showCertificateGenerateButton =
                            $verifyStatus === 'Approved' &&
                            $applicationStatus === 'Approved' &&
                            $verificationStatus === 'Approved' &&
                    ...
                    @endphp
                    @if ($showCertificateGenerateButton)
                        <button class="rounded-md p-3 text-md font-medium cursor-pointer bg-green-500"
                            wire:click="generateCertificate('{{ $details->id }}')">Generate Certificate</button>
                    @endif

                    @if ($details->certificate_remark && $details->certificate_status === 'changes_required')
                        <div class="rounded-md p-2 text-md font-medium cursor-pointer bg-blue-200"
                            wire:click="setDocument('{{ $details->id }}')">Certificate Remark :
                            {{ $details->certificate_remark }}</div>
                    @endif
                </div>
            </div>
        @endforeach
            ...
            <div class="flex justify-between items-center">
                <div class="flex gap-2">
                    @if ($checkApproved == 'Approved')
                        <flux:modal.close>
                            <flux:button
                                x-on:click="$wire.updateFileStatus('{{ $detailId }}','{{ $fileName }}','Rejected')"
                    ...
                    @if ($checkApproved == 'Rejected')
                        <flux:modal.close>
                            <flux:button variant="primary"
                                x-on:click="$wire.updateFileStatus('{{ $detailId }}','{{ $fileName }}','Approved')">
                                Approve</flux:button>
                        </flux:modal.close>
                    @endif
                    @if ($checkApproved == '')
    ...
    <flux:modal wire:model="verificationModal" class="!max-w-2xl w-full">
        <div class="space-y-6">
            ...
            <div class="flex justify-between">
                <flux:modal.close>
                    <flux:button variant="primary" x-on:click="$wire.approveDetail('{{ $detailId }}')">Approve
                    </flux:button>
                </flux:modal.close>
            ...
            @if ($isRejecting)
                <div class="flex justify-end mt-2">
                    <flux:modal.close>
                        <flux:button x-on:click="$wire.rejectDetail('{{ $detailId }}')" variant="danger">Confirm
                            Rejection</flux:button>
                    </flux:modal.close>
                </div>
            @endif
        </div>
