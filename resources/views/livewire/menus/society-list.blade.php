        ...
        @foreach ($societies as $society)
            <div class="flex items-center justify-between w-full border-b text-left font-medium dark:text-white text-stone-800 cursor-pointer transition-colors duration-200 hover:bg-gray-200"
                ...
                <div class="p-4 text-left align-top w-full">
                    <div class="flex items-center gap-2 flex-wrap mb-1">
                        <h3 class="font-bold text-lg text-blue-900 dark:text-blue-300 mr-2">{{ $society->society_name }}</h3>
                        <div class="inline-flex items-center rounded-md bg-white border border-gray-300 px-2 py-0.5 text-xs font-mono text-gray-700 shadow-sm" title="Registration Number">
                    ...
                    <div class="flex items-center gap-2 flex-wrap mb-2">
                        ...
                        <span class="inline-flex items-center gap-1 rounded-sm px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider {{ strtolower($society->is_list_of_signed_member_available) === 'yes' ? 'bg-green-100 text-green-700' : 'bg-red-50 text-red-600' }}">
                            <flux:icon.users variant="mini" class="size-3" />
                            Signed Mbrs: {{ $society->is_list_of_signed_member_available ?? 'No' }}
                        </span>
                        
                        <span class="inline-flex items-center gap-1 rounded-sm px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider {{ strtolower($society->is_byelaws_available) === 'yes' ? 'bg-green-100 text-green-700' : 'bg-red-50 text-red-600' }}">
            ...
            <div id="basicAccordion{{ $society->id }}"
                ...
                @if($society->details->isEmpty())
                    <div class="p-8 flex flex-col items-center justify-center text-gray-500 bg-gray-50 dark:bg-gray-800/50">
                            ...
                            <tbody>
                                @foreach ($society->details as $index => $detail)
                                    <tr class="border-b border-neutral-200 dark:border-white/10">
                                        ...
                                        <td class="whitespace-nowrap px-4 py-3">{{ $detail->society->no_of_shares }}
                                        </td>
                                        ...
                                        <td class="whitespace-nowrap px-4 py-3">{{ $detail->signed_member_name }}</td>
                                        <td class="whitespace-nowrap px-4 py-3">
