    ...
    <div class="w-full">
        ...
        <div class="rounded-lg shadow-lg py-4 px-6">
            ...
            <div class="card shadow-sm border-0 mb-6">
                <div class="card-header font-bold text-lg bg-white py-4 border-b">Section 1: Society & Owner Details
                </div>
                <div class="card-body">
                            ...
                            <div class="mt-6 space-y-3">
                                <flux:field>
                                    ...
                                    <flux:text>
                                        @switch($membership_case)
                                            ...
                                            @case('case_c')
                                                Case C: Nominee Succession
                                            @break

                                            @case('case_d')
                                                Case D: Heir Succession
                                            @break
                                        @endswitch
                                    </flux:text>
                                </flux:field>
                                ...
                                @if ($membership_case === 'case_a')
                                    <div class="mt-8 space-y-6 pt-6 border-t border-gray-100">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <flux:field>
                                                <flux:label>Applicant Name</flux:label>
                                                <flux:text>{{ $applicant_name }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Father/Husband Name</flux:label>
                                                <flux:text>{{ $father_husband_name }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Deceased Member Name</flux:label>
                                                <flux:text>{{ $deceased_member_name }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Builder Name</flux:label>
                                                <flux:text>{{ $builder_name }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Age</flux:label>
                                                <flux:text>{{ $age }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Occupation</flux:label>
                                                <flux:text>{{ $occupation }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Monthly Income</flux:label>
                                                <flux:text>₹{{ number_format($monthly_income) }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Residential Address</flux:label>
                                                <flux:text>{{ $residential_addr }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Office Address</flux:label>
                                                <flux:text>{{ $office_addr }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Flat Area (Sq. Meters)</flux:label>
                                                <flux:text>{{ $flat_area_sq_meters }}</flux:text>
                                            </flux:field>
                                        </div>
                                        <div class="space-y-4 p-4 rounded-xl bg-blue-50/30 border border-blue-100">
                                            <flux:label class="font-bold">Other Particulars of Plot/Flat/House:
                                            </flux:label>
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    <flux:label>Person 1 Details</flux:label>
                                                    <flux:text>{{ $other_person_name1 }} -
                                                        {{ $other_property_location1 }}</flux:text>
                                                    <p class="text-sm text-gray-500 mt-1">
                                                        {{ $other_property_particulars1 }}</p>
                                                    <p class="text-xs italic mt-1">Reason: {{ $reason_for_flat1 }}</p>
                                                </div>
                                                @if ($other_person_name2)
                                                    <div>
                                                        <flux:label>Person 2 Details</flux:label>
                                                        <flux:text>{{ $other_person_name2 }} -
                                                            {{ $other_property_location2 }}</flux:text>
                                                        <p class="text-sm text-gray-500 mt-1">
                                                            {{ $other_property_particulars2 }}</p>
                                                        <p class="text-xs italic mt-1">Reason: {{ $reason_for_flat2 }}
                                                        </p>
                                                    </div>
                                                @endif
                                            </div>
                                ...
                                @if ($membership_case === 'case_b')
                                    <div class="mt-8 space-y-6 pt-6 border-t border-gray-100">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <flux:field>
                                                <flux:label>Transferee Name</flux:label>
                                                <flux:text>{{ $transferee_name }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Transferor Name</flux:label>
                                                <flux:text>{{ $transferor_name }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Distinctive No</flux:label>
                                                <flux:text>{{ $distinctive_no_from }} to {{ $distinctive_no_to }}
                                                </flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Building No</flux:label>
                                                <flux:text>{{ $building_no }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Flat Area (Sq. Meters)</flux:label>
                                                <flux:text>{{ $flat_area_sq_meters }}</flux:text>
                                            ...
                                            <flux:field>
                                                <flux:label>Transfer Fee</flux:label>
                                                <flux:text>₹{{ number_format($transfer_fee) }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Transfer Premium</flux:label>
                                                <flux:text>₹{{ number_format($transfer_premium_amount) }}</flux:text>
                                            </flux:field>
                                        </div>
                                        <div class="space-y-2">
                                            <flux:label class="font-bold">Grounds for Transfer:</flux:label>
                                            <ul class="list-disc list-inside text-sm text-gray-600">
                                                @if ($transfer_ground_1)
                                                    <li>{{ $transfer_ground_1 }}</li>
                                                @endif
                                                @if ($transfer_ground_2)
                                                    <li>{{ $transfer_ground_2 }}</li>
                                                @endif
                                                @if ($transfer_ground_3)
                                                    <li>{{ $transfer_ground_3 }}</li>
                                                @endif
                                            </ul>
                                        </div>
                                    </div>
                                ...
                                @if ($membership_case === 'case_c')
                                    <div class="mt-8 space-y-6 pt-6 border-t border-gray-100">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <flux:field>
                                                <flux:label>Deceased Member</flux:label>
                                                <flux:text>{{ $deceased_member_name }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Date of Death</flux:label>
                                                <flux:text>{{ $date_of_death }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>No of Shares</flux:label>
                                                <flux:text>{{ $society_shares }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Occupation</flux:label>
                                                <flux:text>{{ $occupation }}</flux:text>
                                ...
                                @if ($membership_case === 'case_d')
                                    <div class="mt-8 space-y-6 pt-6 border-t border-gray-100">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <flux:field>
                                                <flux:label>Floor No</flux:label>
                                                <flux:text>{{ $floor_no }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Flat Bearing No</flux:label>
                                                <flux:text>{{ $flat_bearing_no }}</flux:text>
                                            </flux:field>
                                            <flux:field>
                                                <flux:label>Inspection Time</flux:label>
                                                <flux:text>{{ $inspection_time_from }} to {{ $inspection_time_to }}
                                                </flux:text>
                                            </flux:field>
                                        </div>
                                        <div class="space-y-2">
                                            ...
                                            <div class="flex flex-wrap gap-4 text-sm">
                                                @if ($heir_1_name)
                                                    <span
                                                        class="bg-gray-100 px-3 py-1 rounded-full">{{ $heir_1_name }}</span>
                                                @endif
                                                @if ($heir_2_name)
                                                    <span
                                                        class="bg-gray-100 px-3 py-1 rounded-full">{{ $heir_2_name }}</span>
                                                @endif
                                                @if ($heir_3_name)
                                                    <span
                                                        class="bg-gray-100 px-3 py-1 rounded-full">{{ $heir_3_name }}</span>
                                                @endif
                                                @if ($heir_4_name)
                                                    <span
                                                        class="bg-gray-100 px-3 py-1 rounded-full">{{ $heir_4_name }}</span>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                @if (count($currentSpecializedLabels) > 0)
                                    <div class="mt-8 pt-6 border-t border-gray-100">
                                        <flux:heading size="sm" class="mb-4">Case Specific Documents
                                        </flux:heading>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            @foreach ($currentSpecializedLabels as $col => $label)
                                                @if ($this->$col)
                                                    <div
                                                        class="flex items-center justify-between p-3 border rounded-xl bg-white shadow-sm">
                                                        <div class="flex items-center gap-3">
                                                            <i class="fa-solid fa-file-pdf text-red-500 text-xl"></i>
                                                            <span
                                                                class="text-sm font-medium">{{ $label }}</span>
                                                        </div>
                                                        <flux:button variant="ghost" size="sm" icon="eye"
                                                            href="{{ asset('storage/society_docs/' . $this->$col) }}"
                        ...
                        @foreach ($standardDocs as $col => $label)
                            @if ($this->$col)
                                <div class="flex items-center justify-between p-3 border rounded-xl bg-white shadow-sm">
                                    ...
                                    <flux:button variant="ghost" size="sm" icon="eye"
                                        href="{{ asset('storage/society_docs/' . $this->$col) }}" target="_blank">View
                                    </flux:button>
                                </div>
                            @endif
                        @endforeach
